...
class UpdateTmsTrip extends StatelessWidget {
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Form(
            ...
            child: GetBuilder<UpdateTmsController>(
              initState: (_) async {
                controller.fetchLocations();
                controller.fetchVehicleNumbers();
                controller.fetchBillingUnits();
                // controller.fetchCargoType();
                controller.fetchTypeofTrip();
                //controller.fetchSegment();

                // Fetch trip history data (New)
                String? tripNo =
                    Get.arguments?['tripNo']; // Get trip number from arguments
                if (tripNo != null) {
                  await controller.fetchTripDetails(tripNo);
                }
              },
              builder: (controller) {
                return Column(
  ...
  Widget buildReadOnlyField(
    ...
    return Padding(
      ...
      child: Obx(() {
        ...
        // Ensure the text update happens after the frame is built
  ...
  Widget buildTextField(
    ...
    bool isLabel = false,
    bool isDefault =
        false, // Add this parameter to check if default value should be used
  }) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: TextFormField(
