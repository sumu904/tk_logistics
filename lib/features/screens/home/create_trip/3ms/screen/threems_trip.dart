          ...
          child: Form(
            ...
            child: GetBuilder<ThreemsController>(
              initState: (_) {
                ...
                tripController.fetchVehicleSizes();
                ...
                if (tripController.locations.isNotEmpty) {
                  tripController.from.value =
                      (userController.user.value?.zone?.isNotEmpty ?? false)
                          ? userController.user.value?.zone
                          : tripController
                              .locations.first; // Set first item as default
              ...
              builder: (controller) {
                return Column(
                  children: [
                      ...
                      children: [
                        Expanded(
                            child: buildSearchableDropdown(
                                "From", tripController.locations, tripController.from,
                                required: true, allowAdd: true)),
                        SizedBox(width: 10),
                        Expanded(
                            child: buildSearchableDropdown(
                                "To", tripController.locations, tripController.to,
                                required: true, allowAdd: true)),
                      ],
                    ),
                    SizedBox(
                    ...
                    Row(
                      children: [
                        Expanded(
                          ...
                          "Loading Points",
                              tripController.loadingPointController,
                          isNumeric: true,
                          isDefault: true,
                        )),
                        SizedBox(
                        ...
                        Expanded(
                            child: buildTextField(
                          "Unloading Points",
                              tripController.unloadingPointController,
                          isNumeric: true,
                          isDefault: true,
                        )),
                      ],
                    ...
                    buildSearchableDropdown("Pick Vendor",
                        tripController.pickSuppliers, tripController.pickSupplier,
                        allowAdd: true, required: true),
                    SizedBox(
                      height: 5,
                    ),
                    Row(children: [
                      Expanded(
                        child: buildTextField(
                            "Vehicle Regn No.", tripController.vehicleNoController,
                            required: true),
                      ),
                      SizedBox(
                        width: 10,
                      ...
                      Expanded(
                          child: buildSearchableDropdown("Vehicle Size",
                              tripController.vehicleSizes, tripController.vehicleSize,
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: buildTextField(
                              'Driver Name', tripController.driverNameController),
                        ),
                        SizedBox(width: 10),
                        Expanded(
                          //print("Driver Phone: ${controller.selectedDriverMobile.value}");  // Print driver phone
                          child: buildTextField("Driver's Phone",
                              tripController.driverPhoneController),
                        ),
                      ],
                    ),
                    SizedBox(
                    ...
                    buildSearchableDropdown("Billing Unit",
                        tripController.billingUnits, tripController.billingUnit,
                        required: true, allowAdd: false),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: buildTextField("Cargo Weight (MT)",
                              tripController.cargoWeightController,
                    ...
                    buildMultiSelectDropdown(
                      ...
                      tripController.cargoTypes,
                      // List of cargo types (RxList<String>)
                      tripController.cargoType,
                      // Selected cargo types (RxList<String>)
                      ...
                      allowAdd:
                          true, // If you want to allow adding new items, set to true
                    ),
                    ...
                    Row(
                      children: [
                        Expanded(
                            child: buildTextField(
                                "Distance (KM)", tripController.distanceController,
                                isNumeric: true)),
                        SizedBox(width: 10),
                        Expanded(
                            child: buildTextField("Service Charge (BDT)",
                                tripController.serviceChargeController,
                                isNumeric: true)),
                      ],
                    ),
                      ...
                      children: [
                        Expanded(
                          child: buildDateTimeField(
                            "Pickup Time",
                            tripController.pickupDate,
                            controller: tripController.pickupDateController,
                            required: true,
                          ),
                        ),
                        SizedBox(width: 10),
                        Expanded(
                          child: buildDateTimeField(
                            "Est. Drop-off Time",
                            tripController.dropOffDate,
                            controller: tripController.dropOffDateController,
                            required: true,
                          ),
                        ),
                      ],
                    ...
                    buildTextField("Special Note", tripController.noteController,
                        isLabel: true),
                    SizedBox(
                      height: 10,
                    ),
                                      ...
                                      TextButton(
                                        onPressed: () async {
                                          ...
                                          await loadingController.runWithLoader(
                                            ...
                                            action: () async {
                                              await tripController.createTrip();  // Just await it

                                              _formKey.currentState?.reset();     // Reset form validation
                                              tripController.clearFormFields();
                                              tripController.refreshFormUI();// Clear input fields
                                            },
                                          );
                                        },
                                        child: Text("Yes",
    ...
    return Padding(
      ...
      child: Obx(() {
        ...
        return TextFormField(
          ...
          onTap: () async {
            await tripController.pickDateTime(date);
          },
        );
      }),
    );
  ...
  Widget buildMultiSelectDropdown(
    ...
    return Obx(() {
      ...
      return TextFormField(
        ...
        readOnly: true,
        style:
            quicksandSemibold.copyWith(fontSize: Dimensions.fontSizeFourteen),
        onTap: () {
          showMultiSelectDialog(
              label, items, selectedValues, controller, onSelected, allowAdd);
        },
        decoration: InputDecoration(
          labelText: label,
          labelStyle:
      ...
      child: ValueListenableBuilder<TextEditingValue>(
        ...
        builder: (context, value, child) {
          ...
          return TextFormField(
            ...
            inputFormatters:
                isDefault ? [FilteringTextInputFormatter.digitsOnly] : [],
            style: quicksandSemibold.copyWith(
              fontSize: Dimensions.fontSizeFourteen,
            ),
            onChanged: (value) {
