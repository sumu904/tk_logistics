...
class FuelEntryList extends StatelessWidget {
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: controller.entryListDate.value,
      firstDate: DateTime(2000),
      lastDate: DateTime.now(),
    );

    if (picked != null) {
      controller.pickDateForEntryList(picked); // Correct method to update entryListDate
      controller.fetchFuelEntries(); // Fetch entries for the selected date
            ...
            children: [
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      SizedBox(width: 15),
                      Obx(() => Text(
                        DateFormat('MMMM dd, yyyy').format(controller.entryListDate.value),
              ...
              Container(
                ...
                child: Obx(() {
                  ...
                  String formattedDate = "${DateFormat('yyyy-MM-dd').format(controller.entryListDate.value)}T00:00:00";
          
          // Filter fuel entries
                  var filteredEntries = controller.fuelEntries
                      .where((entry) {
                    final entryDate = entry['xdate'];
                    final entryEmail = entry['zemail'];
          
                    if (entryDate == null || entryEmail == null) return false;
          
                    final formattedEntryDate = "${DateFormat('yyyy-MM-dd').format(DateTime.parse(entryDate))}T00:00:00";
          
                    return entryEmail == userController.user.value?.username &&
